...
are two limits of a single model, with the superradiant solid as the coexistence region.
\end{abstract}
...
Route~I is a factor $5.1$ better at equal atom number, for a structural reason worth stating
plainly: the lasing route inverts and uses \emph{all} $N$ atoms, whereas Route~II linearises
...
Hysteresis & yes (scan-direction dependent) & no (second order) & no (second order)\\
...
\begin{enumerate}
  \item \textbf{Build Route~I.} It is the cheaper experiment, it reaches the lower floor, and
        its drift immunity is the defensible advantage. The apparatus already exists:
