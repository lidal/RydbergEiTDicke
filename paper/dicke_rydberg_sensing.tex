...
090402 (2013)] showed that Rydberg polaritons in a cavity realise a generalised Dicke
model with a superradiant phase transition. This note works out what happens when the two
are combined \emph{and the transition itself is used as the sensing element}: the microwave
...
\paragraph{A note on sourcing.} The literature survey in Sec.~\ref{sec:prior} was assembled
from search-engine abstracts and metadata; the network policy of the machine this was
prepared on blocked direct access to arXiv, APS and Optica full texts. Titles, authors,
venues and the quoted headline numbers were each cross-checked against at least two
independent search results, but the reasoning inside the cited papers has not been read
first-hand. Any claim below about what a cited paper \emph{does not} do should be
re-verified before it appears in a manuscript.
...
second control field is the only addition.
...
Ref.~\cite{Jing2020} achieves $55~\mathrm{nV\,cm^{-1}Hz^{-1/2}}$ and the cavity-enhanced
critical measurement of Ref.~\cite{Wang2025} reports $2.6~\mathrm{nV\,cm^{-1}Hz^{-1/2}}$.}
\end{table}
...
\begin{itemize}
  \item \textbf{For Protocols A and C, Rydberg interactions are a nuisance.} The clean
        ...
        atom number. Interactions also dephase the collective mode, raising $\gamma$.
...
The recent proposal of an anisotropic Dicke--Ising model in driven Rydberg
arrays~\cite{DickeIsing2025} covers the equilibrium phase structure of this regime; it does
not address sensing.
...
attributing the gain to cavity narrowing of the transmission features. The same group then
moved to the collective regime: Ref.~\cite{Yang2020} uses \emph{collective Rabi splitting}
--- i.e.\ $g_0\sqrt N$ --- and reports about $7\times$ improvement over plain EIT, and
Ref.~\cite{Wang2023} reports $196.7\times$ over a bare atomic medium and $26.2\times$ over
weak coupling. Experimentally, Ref.~\cite{Liu2025} demonstrates 18~dB of power-sensitivity
enhancement using a microwave (not optical) resonator.
...
\emph{counter-rotating} terms and the phase transition they enable --- Refs.~\cite{Yang2020,
Wang2023} are Tavis--Cummings physics in the normal phase, with no transition and no
critical point.

\subsection{Critical Rydberg metrology --- the closest competitor}

Ding, Liu, Shi, Guo, Mølmer and Adams~\cite{Ding2022} demonstrated many-body enhanced
...
\begin{itemize}
  \item Refs.~\cite{Ding2022,Wang2025} use the \emph{dissipative, Rydberg-interaction-driven}
        ...
        temperature, density and stray fields, and cannot be tuned quickly.
...
self-organisation of a BEC. Cavity Rydberg polaritons have been observed~\cite{Ningyuan2016},
...
On this survey, the following combination does not appear in the literature:
\begin{enumerate}
  \item Using the \emph{Dicke} (light--matter) superradiant transition, rather than a
        Rydberg-interaction transition, as the critical resource for microwave electrometry.
  \item The \emph{null / threshold-crossing} read-out of Protocol~B, in which the measurand
        is the laser detuning that holds the system at threshold --- SI-traceable, and with a
        discriminant not limited by the EIT linewidth.
  \item The explicit gain--bandwidth characterisation of a critical Rydberg sensor, including
        the finding that there is a wide window of gain before critical slowing down is paid
        for.
\end{enumerate}
Item 2 is the strongest. Items 1 and 3 are supporting.
...
\begin{itemize}
  ...
  \item \textbf{Criticality amplifies drift as well as signal.} Protocol A has no
        ...
        laser intensity noise or $\delta\omega$ from cavity drift. Protocol C's IF encoding
        and Protocol B's null lock are the mitigations, and they are not optional.
  ...
  \item \textbf{Engineering overhead.} Two Raman channels plus a high-finesse cavity plus
        ...
        makes Rydberg electrometry commercially interesting.
...
\begin{enumerate}
  ...
  \item \textbf{Cheap experimental precursor.} Protocol~C in the \emph{Tavis--Cummings}
        regime --- no counter-rotating terms, no phase transition --- is essentially
        Ref.~\cite{Yang2020}'s apparatus plus a microwave LO. It would demonstrate the
        superheterodyne-in-a-cavity gain and de-risk everything except the Dicke engineering.
  \item \textbf{Only then} add the second Raman channel and go for the transition.
\end{enumerate}
